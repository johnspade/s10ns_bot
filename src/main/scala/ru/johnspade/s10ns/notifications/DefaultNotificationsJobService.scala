...
class DefaultNotificationsJobService[F[_]: Concurrent: Temporal: Logging, D[_]: Monad](
...
)(implicit transact: D ~> F, monadError: MonadError[F, Throwable])
    extends NotificationsJobService[F] {
  def executeTask()(implicit bot: Api[F]): F[Unit] = {
    def sendNotification(s10nWithUser: (Subscription, User), timestamp: Instant) = {
      ...
      user.chatId
        .map { chatId =>
          s10nListMessageService
            .createSubscriptionMessage(user.defaultCurrency, s10n)
            .flatMap { message =>
              sendMessage(
