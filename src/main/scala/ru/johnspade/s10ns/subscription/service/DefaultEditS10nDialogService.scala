...
class DefaultEditS10nDialogService[F[_] : Sync, D[_] : Monad](
...
)(private implicit val transact: D ~> F) extends EditS10nDialogService[F] {
  override def onEditS10nNameCb(user: User, cb: CallbackQuery, data: EditS10nName): F[ReplyMessage] =
    onEditS10nDialogCb(
      user = user,
      cb = cb,
  ...
  override def saveName(user: User, dialog: EditS10nNameDialog, text: Option[String]): F[RepliesValidated] =
    validateText(text)
      .andThen(name => validateNameLength(SubscriptionName(name)))
      .traverse(editS10nDialogFsmService.saveName(user, dialog, _))

  override def onEditS10nAmountCb(user: User, cb: CallbackQuery, data: EditS10nAmount): F[ReplyMessage] = {
  ...
  override def onEditS10nCurrencyCb(user: User, cb: CallbackQuery, data: EditS10nCurrency): F[ReplyMessage] =
    onEditS10nDialogCb(
      user = user,
      cb = cb,
      s10nId = data.subscriptionId,
  ...
  override def onEditS10nOneTimeCb(user: User, cb: CallbackQuery, data: EditS10nOneTime): F[ReplyMessage] =
    onEditS10nDialogCb(
      user = user,
      cb = cb,
      s10nId = data.subscriptionId,
  ...
  override def saveBillingPeriodUnit(cb: CallbackQuery, data: PeriodUnit, user: User, dialog: EditS10nOneTimeDialog): F[List[ReplyMessage]] =
  ...
  override def onEditS10nBillingPeriodCb(user: User, cb: CallbackQuery, data: EditS10nBillingPeriod): F[ReplyMessage] =
    onEditS10nDialogCb(
      user = user,
      cb = cb,
      s10nId = data.subscriptionId,
  ...
  override def saveBillingPeriodUnit(cb: CallbackQuery, data: PeriodUnit, user: User, dialog: EditS10nBillingPeriodDialog): F[List[ReplyMessage]] =
    editS10nDialogFsmService.saveBillingPeriodUnit(user, dialog, data.unit)
  ...
  override def onEditS10nFirstPaymentDateCb(user: User, cb: CallbackQuery, data: EditS10nFirstPaymentDate): F[ReplyMessage] = {
